# -*- coding: utf-8 -*-
"""Kimch_DNN_augmentation.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1G_Gvp3pR9RS11m5DJa5mrNLGdRBEWNfA

#pytorch DNN image classification
"""
...
kimch_train = datasets.ImageFolder(root=os.path.join(dataset_path, 'train'), transform=augmentation_trans)
kimch_valid = datasets.ImageFolder(root=os.path.join(dataset_path, 'val'), transform=augmentation_trans)
...
class kimchi_MLP(nn.Module):
    def __init__(self, num_classes):
        super(kimchi_MLP, self).__init__()

        #input image size = 64 x 64 x 3
        #Adding Dropout
        #Adding more layers
        self.fc1 = torch.nn.Sequential(torch.nn.Linear(64 * 64 * 3, 1024,  bias = True),
            torch.nn.ReLU(),
            torch.nn.Dropout(p=0.25))

        self.fc2 = torch.nn.Sequential(torch.nn.Linear(1024, 512,  bias = True),
            torch.nn.ReLU())

        self.fc3 = torch.nn.Sequential(torch.nn.Linear(512, 256, bias = True),
            torch.nn.ReLU(),
            torch.nn.Dropout(p=0.25))

        self.fc4 = torch.nn.Sequential(torch.nn.Linear(256, 128, bias = True),
            torch.nn.ReLU())

        self.fc5 = torch.nn.Linear(128, num_classes, bias = True)
...
model = kimchi_MLP(num_classes = 11).to(device)
