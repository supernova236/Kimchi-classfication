...
Original file is located at
    https://colab.research.google.com/drive/1NgCl_lGNIXHZ6fE776L4U10SyPkzJqBT
...
class kimchi_MLP(nn.Module):
    def __init__(self, num_classes):
        ...
        self.fc2 = torch.nn.Sequential(torch.nn.Linear(1024, 256,  bias = True),
            torch.nn.ReLU())

        self.fc3 = torch.nn.Linear(256, num_classes, bias = True)

    def forward(self, x):
        out = x.view(x.size(0), -1)
        #out = x.view(-1, 64*64*3)
        out = self.fc1(out)
        out = self.fc2(out)
        out = self.fc3(out)
        return out
...
# Loss function and optimizer for Kimchi model
